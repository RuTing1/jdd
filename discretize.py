...
class BaseDiscretizer(BaseEstimator,TransformerMixin):
    ...
    def __init__(self,feature_names=None,fill_na=-1,return_numeric=True,decimal=2):
...
class QuantileDiscretizer(BaseDiscretizer):
    def __init__(self,feature_names=None,quantiles=[25,50,75],fill_na=-1,return_numeric=True,decimal=2):
        '''
        根据样本分位数离散化。
        feature_names：需要离散化的变量列表，非负整数或者字符串。
        quantiles：离散化的分位点列表。
        '''
        BaseDiscretizer.__init__(self,feature_names=feature_names,fill_na=fill_na,return_numeric=return_numeric,decimal=decimal)
...
class SimpleBinsDiscretizer(BaseDiscretizer):
    def __init__(self,feature_names=None,bins=10,fill_na=-1,return_numeric=True,decimal=2):
        '''
        根据等分点离散化。
        feature_names：需要离散化的变量列表，非负整数或者字符串。
        bins：等分区间数。
...
class EntropyDiscretizer(BaseDiscretizer):
    def __init__(self,feature_names=None,max_depth=3,fill_na=-1,return_numeric=True,decimal=2,**kwds):
        '''
        根据单变量决策树离散化。
        feature_names：需要离散化的变量列表，非负整数或者字符串。
