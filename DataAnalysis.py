...
def error_df(data, cols=None):
    ...
    error_data = pd.DataFrame(index=cols, columns=['ErrorId','ErrorNum','ErrorPercent'])
    for column in cols:
        error_series = data[column]
        zscore = (error_series - error_series.mean()) / error_series.std()
        error_count = (zscore.abs() > 3.5).sum()
        error_id = error_series[(zscore.abs() > 3.5)].index
        error_percent = error_count.astype(float) / len(error_series)
        error_data.ix[column,:] = error_id, error_count, error_percent
    return error_data
...
#Distribution
def dist_plot(data, cols=None, feature_cate=None, figsize=(18,8), close=True, show_last=False, verbose=False):
    '''
...
def plot_doubleXY_Mean(X, cols_h=None, cols_v=None, Y_cont=None, Y_cate=None, feature_cate=None, quantiles=None, cuts=None,
    ...
    feature_cont=np.sort(np.unique(feature_cont))
