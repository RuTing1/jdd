...
- **6.Regularization parameter lambda 选择多大合适**  <br/>
  ...
  &nbsp;&nbsp;注意：不要让正则化项盖过数据项,有时候会出现这个问题，因为损失函数是数据损失部分与正则化部分的求和。如果出现，主要的梯度来源于正则化项，那这样根本就做不到正常的梯度回传和参数迭代更新。所以在检查数据部分的实现是否正确时应该先关闭正则化部分(系数设为0)，再检查。
- **7.激励函数如何选择**  <br/>
...
# 5.[keras结构](https://blog.csdn.net/sinat_26917383/article/details/72857454)
- kears网络结构
