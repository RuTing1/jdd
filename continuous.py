...
class BaseContinuous(BaseEstimator,TransformerMixin):
    ...
    method：具体使用的连续化方法名称。
    crosstabs：列联表或列联表字典；
               若fit的X为Series，crosstabs则为列联表DataFrame，对应变量不同类别取值及不同响应变量下的样本数量；
               若fit的X为DataFrame，crosstabs则为字典，键为变量名，值为列联表DataFrame。
    single：是否为单一变量。
    maps：映射字典或映射字典的字典；
          若fit的X为Series，maps则为映射字典，键为变量不同取值，值为对应的连续化值（如某个类别取值的WOE）；
          若fit的X为DataFrame，maps则为映射字典的字典，键为变量名，值为映射字典（同fit的X为Series时的maps值）。
    """
    ...
    def get_crosstab(self,X,y):
        ...
        X：DataFrame，或Series。
        y：Series，index需要与X对应（暂时只支持0-1）。
        返回列联表DataFrame（X为Series）或字典（X为DataFrame，键为变量名，值为列联表DataFrame）
        '''
        data=X.copy()
        if len(data.shape)==1:
            ...
            for feature in feature_names:
                result[feature]=pd.crosstab(data[feature],y)
        return result

    def plot(self,close=True,show_last=True):
        '''
        ...
        if self.single is True:
            ...
            result['']=fig
            if close and not show_last:
                plt.close('all')
        else:
            for i,feature in enumerate(self.maps):
                ...
                result[feature]=fig
                if close and (not show_last) and i<len(self.maps)-1:
                    plt.close('all')
        return result
...
class ProbContinuous(BaseContinuous):
    def __init__(self,feature_names=None):
        ...
        feature_names：需要连续化的变量列表，非负整数或者字符串。
        '''
        BaseContinuous.__init__(self,feature_names=feature_names)
        self.method='Prob'
...
class OneHotContinuous(BaseEstimator,TransformerMixin):
    ...
    def __init__(self,feature_names=None,feature_tolabel=None,drop_others=False,sep='=',handle_na=99):
        self.feature_names=feature_names
    ...
    def fit(self,X,y=None,**kwds):
        ...
        self.label_continuous_=LabelContinuous(feature_names=self.feature_tolabel)
        ...
        self.one_hot_enc_=OneHotEncoder(sparse=False,handle_unknown='ignore',**kwds)
    ...
    def transform(self,X):
        ...
        X：DataFrame。
        返回转换后的DataFrame，前几列为独热编码变量，其他的变量按列合并到后面（如果self.drop_others为False）。
        '''
        ...
        if self.drop_others:
            ...
            return pd.concat([Xnew,X_others],axis=1)
...
def test():
    ...
    #改写的独热编码
    X_train=pd.DataFrame(np.random.choice(['a','b','c','d'],[1000000,3]),columns=['x1','x2','x3'])
    X_test=pd.DataFrame(np.random.choice(['a','b','c','d'],[5,3]),columns=['x1','x2','x3'])
    X_test[X_test=='d']='f'
